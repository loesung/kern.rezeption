...
function receiverTunnels($, _, ipc, tunnels){
    ...
    this.list = function(){
        var ret = {};
        for(var i in idList){
            var parsed = $.security.object.check.tunnel.id(idList[i]);
    ...
    this.setTunnelInfo = function(feed){
        ...
        for(var i in split){
            try{
                ...
                id = $.security.object.check.tunnel.id(exec[1]);
                desc = $.security.object.get.tunnel.description(exec[2]);
                if(!(id && desc)) continue;
//              console.log('**', id, desc, exec[1], exec[2]);
        ...
        if(newList.length > 0){
            ...
            for(var i in newList){
                receiverID = newList[i][0].identity.toLowerCase();
                if(undefined == refresh[receiverID])
                    refresh[receiverID] = [];
                refresh[receiverID].push({
                    id: newList[i][2],
                    description: newList[i][1],
                });
            };
            tunnels = refresh;
            lastUpdate = new Date();
